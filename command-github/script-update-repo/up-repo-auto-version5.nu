...
# Ejemplos:
#   nu update-repo.nu .
#   nu update-repo.nu ~/java-proyects -m "feat: cambios" --reindex
#   nu update-repo.nu . --public     # si crea repo nuevo con gh, será público
...
def-env start_ssh_agent [] {
  ...
  let-env SSH_AUTH_SOCK = (
    $out | lines
    | where {|x| $x | str starts-with "SSH_AUTH_SOCK="}
    | str replace "SSH_AUTH_SOCK=" ""
    | split row ";" | get 0 | str trim
  )
  let-env SSH_AGENT_PID = (
    $out | lines
    | where {|x| $x | str starts-with "SSH_AGENT_PID="}
