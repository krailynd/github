...
# Requisitos: git, gh (opcional), ssh-agent/ssh-add
# Uso:
#   nu git-menu.nu
#   nu git-menu.nu --ruta ./mi/carpeta
...
# --- FIX ROBUSTO: exporta variables del ssh-agent al entorno ---
def-env start_ssh_agent [] {
  ...
  let sock = (
    $out
    | lines
    | where { |it| $it =~ '^SSH_AUTH_SOCK=' }
    | first
    | default ""
    | str replace --regex '^SSH_AUTH_SOCK=' ''
    | split row ';'
    | first
    | default ""
    | str trim
  ...
  let pid = (
    $out
    | lines
    | where { |it| $it =~ '^SSH_AGENT_PID=' }
    | first
    | default ""
    | str replace --regex '^SSH_AGENT_PID=' ''
    | split row ';'
    | first
    | default ""
    | str trim
